...
def encode_numeric_range(df, name, normalized_low=0, normalized_high=1,
    ...
    for i in range(len(df[name])):
        df[name][i] = ((df[name][i] - data_low) / (data_high - data_low)) \
...
def to_sequences(seq_size, data1 ,data2):
    x = []
    y = []
    for i in range(len(data1)-SEQUENCE_SIZE-1): #(4392-7-1 = 4384)
        #print(i)
        window = data1[i:(i+SEQUENCE_SIZE)] #(i-i+7)
...
value = np.zeros(1000*5)
value = value.reshape(1000,5)
...
for i in range(1000):
    ...
    value[i][1] = df["high"][i]
    value[i][2] = df["low"][i]
    value[i][3] = df["close"][i]
    value[i][4] = df["volume"][i]
...
df_stock_close = df["close"]
df_stock_close = np.array(df_stock_close)
...
#Preparing x and y
SEQUENCE_SIZE = 7
x,y = to_sequences(SEQUENCE_SIZE, value, df_stock_close)
print("Shape of x: {}".format(x.shape))
print("Shape of y: {}".format(y.shape))
x_NN = x.reshape(len(value)-SEQUENCE_SIZE-1,SEQUENCE_SIZE*5)
y_NN = y
