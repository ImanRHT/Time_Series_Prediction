...
k = []
for i in range(1000, 2000): ##len(f)-1):
    k.append(float(a["close"][i]))
...
# Fit LSTM model
history = m.fit(xin, next_X, epochs = 500, batch_size = 100,verbose=0)

plt.figure()
plt.ylabel('loss')
plt.xlabel('epoch')
plt.semilogy(history.history['loss'])
plt.savefig("_D11.png")
